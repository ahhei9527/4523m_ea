...
<div class="dashboard-container">
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:2rem; flex-wrap:wrap; gap:1rem;">
        <h3>Showing <?= count($products) ?> products</h3>
        <form method="GET" action="">
            <label>Sort by price:</label>
            <select name="sort" onchange="this.form.submit()">
                <option value="price_asc"  <?= ($sort ?? 'price_asc') === 'price_asc' ? 'selected' : '' ?>>Low to High</option>
                <option value="price_desc" <?= ($sort ?? 'price_asc') === 'price_desc' ? 'selected' : '' ?>>High to Low</option>
            </select>
        </form>
    </div>
            ...
            <?php foreach ($products as $product): ?>
                <div class="product-card">
                    ...
                    <div class="product-info">
                        ...
                        <p class="desc"><?= htmlspecialchars(substr($product['fdesc'] ?? 'Beautiful furniture piece', 0, 80)) ?>...
                        ...
                        <form method="POST" action="" class="add-to-cart-form">
                            ...
                            <div style="display:flex; align-items:center; gap:0.8rem; justify-content:center; margin-top:1rem;">
                                <input type="number" name="qty" value="1" min="1" style="width:60px; padding:0.4rem; text-align:center;">
