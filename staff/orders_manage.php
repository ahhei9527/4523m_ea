...
// Status options (same as your data dictionary)
$status_options = [
    0 => 'Rejected',
    1 => 'Open',
    2 => 'Processing',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
    ...
    if ($new_status === 0 && empty($reject_reason)) {
        ...
        if ($stmt->execute()) {
            $message = "Order #$oid status updated to " . $status_options[$new_status] . ".";
        } else {
...
$orders = [];
$stmt = $conn->prepare("
    SELECT o.oid, o.odate, o.ototalamount, o.odeliverydate, o.odeliveraddress, o.ostatus,
           c.cname, c.ctel
    FROM Orders o
...
<div class="dashboard-container">
    ...
    <?php else: ?>
        <?php foreach ($orders as $order): ?>
            <div style="background:white; border-radius:12px; box-shadow:0 4px 15px rgba(0,0,0,0.08); margin-bottom:2rem; padding:1.5rem;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1rem; flex-wrap:wrap; gap:1rem;">
                    <div>
                        <h3 style="margin:0;">Order #<?= $order['oid'] ?></h3>
                        <small>Placed: <?= date('d M Y H:i', strtotime($order['odate'])) ?></small><br>
                        <small>Customer: <?= htmlspecialchars($order['cname']) ?> (<?= htmlspecialchars($order['ctel'] ?: 'No phone') ?>)</small>
                    </div>
                    <div>
                        <span class="badge bg-<?= ['danger','warning','info','primary','secondary','success'][$order['ostatus']] ?? 'secondary' ?>">
                ...
                <!-- Status update form -->
                <form method="POST" action="" style="margin-top:1.5rem; display:flex; flex-wrap:wrap; gap:1rem; align-items:flex-end;">
